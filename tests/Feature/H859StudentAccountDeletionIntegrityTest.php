...
class H859StudentAccountDeletionIntegrityTest extends TestCase
{
    ...
    public function test_user_management_delete_hard_deletes_student_login_and_preserves_every_history_boundary(): void
    {
        $historical = [
        ...
        $this->get(route('users.index'))
            ->assertOk()
            ->assertDontSee('USR-STUDENT-H859');
        ...
        $this->assertSame(1, $this->users->deleteCalls);
        $this->assertSame(0, $this->students->deleteCalls);
        $this->assertGreaterThanOrEqual(2, $this->students->fetchCalls, 'MASTER_STUDENT must be read again after the account deletion.');
    ...
    public function test_student_role_without_profile_is_hard_deleted_without_fake_student_identity(): void
    {
        $this->users->rows[] = [
        ...
        $this->assertNull($this->users->findById('USR-UNMAPPED-H859'));
    ...
    private function bindProtectedHistoryRepositories(): void
    {
        ...
        $this->app->instance(
            ...
            new H859NotificationRepository()
        );
    }
...
class H859NotificationRepository implements \App\Interfaces\GoogleSheets\NotificationRepositoryInterface
{
