...
>
    ...
    <!-- Filters -->
    <x-universal.filter-container>
        <div class="flex-grow min-w-[200px]">
            <x-universal.search-input name="search" placeholder="Cari siswa atau ID..." value="{{ request('search') }}" />
        </div>
        
        <x-universal.filter-select name="class_id" :options="['' => 'Semua Kelas'] + ($classOptions ?? [])" value="{{ request('class_id') }}" />
        
        <div class="flex items-center gap-2">
            <x-universal.filter-date name="date" value="{{ request('date', date('Y-m-d')) }}" />
            <span class="text-slate-400 font-bold">-</span>
            <x-universal.filter-date name="date_end" value="{{ request('date_end') }}" />
        </div>
        
        <x-universal.filter-select name="status" :options="['' => 'Semua Status', 'Hadir' => 'Hadir', 'Terlambat' => 'Terlambat', 'Izin' => 'Izin', 'Sakit' => 'Sakit', 'Alpha' => 'Alpha']" value="{{ request('status') }}" />
        <x-universal.filter-select name="sort" :options="['desc' => 'Terbaru', 'asc' => 'Terlama']" value="{{ request('sort') }}" />
        <x-universal.filter-button />
        <x-universal.export-button url="{{ route('attendances.export.csv') }}" type="csv" />
    </x-universal.filter-container>
                                    ...
                                    <table class="w-full text-left text-sm">
                                        ...
                                        <tbody class="divide-y divide-slate-100">
                                            @forelse($item['Students'] as $student)
                                                @php
                                                    $status = strtolower($student['Status'] ?? 'hadir');
                                                    $badgeColor = match($status) {
                                                        'hadir', 'present' => 'green',
                                                        'terlambat', 'late' => 'yellow',
                                                        'sakit', 'sick' => 'yellow',
                                                        'izin', 'leave', 'permission' => 'blue',
                                                        'alpha', 'absent', 'alpa' => 'red',
                                                        default => 'slate',
                                                    };
                                                    $displayStatus = match($status) {
                                                        ...
                                                        'absent', 'alpa' => 'Alpa',
                                                        default => ucfirst($student['Status'] ?? 'Hadir'),
                                                    };
                                                @endphp
                                                <tr class="hover:bg-slate-50/80 transition-colors">
                                                    ...
                                                    <td class="px-4 py-3 text-slate-600 text-xs">{{ $student['Time_In'] ?? '-' }}</td>
                                                    <td class="px-4 py-3 text-slate-600 text-xs">{{ $student['Time_Out'] ?? '-' }}</td>
                                                    <td class="px-4 py-3 text-slate-500 text-xs truncate max-w-[150px]">{{ $student['Notes'] ?? '-' }}</td>
                                                    <td class="px-4 py-3 text-right">
                                                        <div class="flex justify-end gap-1">
                                                            <a href="{{ route('attendances.show', $student['Attendance_ID'] ?? 1) }}" class="text-xs font-bold text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100 px-2 py-1 rounded">Detail</a>
                                                            <a href="{{ route('attendances.edit', $student['Attendance_ID'] ?? 1) }}" class="text-xs font-bold text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 px-2 py-1 rounded">Edit</a>
                                                        </div>
                                                    </td>
                                                </tr>
                                            @empty
                            ...
                            <div class="bg-white border border-slate-200 rounded-xl p-3 shadow-sm">
                                ...
                                <div class="flex items-center gap-4 mt-3">
                                    <div>
                                        ...
                                        <p class="text-xs font-bold text-slate-700">{{ $student['Time_In'] ?? '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[9px] font-black text-slate-400 uppercase">Check-Out</p>
                                        <p class="text-xs font-bold text-slate-700">{{ $student['Time_Out'] ?? '-' }}</p>
                                    </div>
                                    <div class="ml-auto">
                                        <a href="{{ route('attendances.show', $student['Attendance_ID'] ?? 1) }}" class="inline-flex items-center justify-center p-1.5 bg-indigo-50 text-indigo-600 rounded-lg border border-indigo-100">
...
echo "Updated resources/views/academic/attendances/index.blade.php\n";
