...
@section('header', 'Developer Panel')

@section('content')
<div class="space-y-6">
    <!-- Header Summary -->
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            <h2 class="text-2xl font-extrabold text-gray-900">Sistem Internal & Metrik</h2>
            <p class="text-sm font-medium text-gray-500 mt-1">Pantau status arsitektur WMS dan kemajuan setiap modul.</p>
        </div>
        <div class="flex items-center gap-4">
            <div class="text-right">
                <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Total Progress</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1 space-y-6">
                ...
                <div class="p-6">
                    <ul class="space-y-4">
                        <li class="flex justify-between items-center">
                            <span class="text-sm font-medium text-gray-600">WMS Version</span>
                            <span class="text-sm font-bold text-gray-900 bg-gray-100 px-3 py-1 rounded-lg">{{ $metrics['version'] }}</span>
                        </li>
                        <li class="flex justify-between items-center">
                        ...
                        <li class="flex justify-between items-center">
                            <span class="text-sm font-medium text-gray-600">Google Sheets DB</span>
                            <span class="text-sm font-bold text-green-700 bg-green-50 border border-green-200 px-3 py-1 rounded-lg flex items-center">
                                <span class="w-2 h-2 rounded-full bg-green-500 mr-2"></span> {{ $metrics['google_sheets_status'] }}
                            </span>
                        </li>
                        <li class="flex justify-between items-center">
                            <span class="text-sm font-medium text-gray-600">Total Audit Logs</span>
                        ...
                        <li class="flex justify-between items-center">
                            <span class="text-sm font-medium text-gray-600">Known Bugs</span>
                            <span class="text-sm font-bold text-gray-700 bg-gray-100 border border-gray-200 px-3 py-1 rounded-lg">{{ $metrics['bug_count'] }}</span>
                        </li>
                    </ul>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl shadow-lg border border-gray-700 overflow-hidden text-white p-6 relative">
                ...
                <h3 class="text-lg font-bold mb-2">Development Note</h3>
                ...
                <div class="flex gap-4">
                    <div>
                        ...
                        <p class="text-xs text-gray-400 font-medium uppercase tracking-widest">APIs</p>
                    </div>
                    <div>
                        <p class="text-2xl font-black">{{ $metrics['total_tables'] }}</p>
                        <p class="text-xs text-gray-400 font-medium uppercase tracking-widest">Tables</p>
                    </div>
                    <div>
                        <p class="text-2xl font-black">{{ $metrics['total_modules'] }}</p>
                        <p class="text-xs text-gray-400 font-medium uppercase tracking-widest">Modules</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden h-full flex flex-col">
                ...
                <div class="p-0 flex-1 overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-white">
                            <tr>
                                <th scope="col" class="px-8 py-4 text-left text-xs font-extrabold text-gray-500 uppercase tracking-wider">Nama Modul</th>
                                <th scope="col" class="px-8 py-4 text-right text-xs font-extrabold text-gray-500 uppercase tracking-wider">Status Implementasi</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                <td class="px-8 py-4 whitespace-nowrap">
                                    <div class="text-sm font-bold text-gray-900 font-mono tracking-tight">{{ $module['name'] }}</div>
                                </td>
                                <td class="px-8 py-4 whitespace-nowrap text-right">
                                    ...
                                    @else
                                        <span class="inline-flex items-center text-sm font-bold text-gray-400">
                                            <svg class="w-5 h-5 mr-1.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                            PENDING
                                        </span>
                                    @endif
                                </td>
                ...
                <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50 text-xs text-center font-medium text-gray-500">
