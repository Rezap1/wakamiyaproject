...
@php
    $formattedKpi = [
        ...
        ['title' => 'Penerimaan Bulan Ini', 'value' => 'Rp '.number_format($kpi['revenue_this_month'] ?? 0, 0, ',', '.'), 'icon' => 'trending-up', 'color' => 'emerald'],
...
<div class="space-y-6" data-finance-dashboard>
    <x-dashboard.action-center 
        title="Dasbor Keuangan"
        description="Pusat kendali keuangan WMS."
        :kpi="$formattedKpi"
        :quick-actions="$quickActions"
        :reminders="$reminders ?? []"
        :recent-activities="$recentActivities ?? []"
        :show-salary-card="false"
    >
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
                <h4 class="text-base font-bold text-slate-800 mb-4">Arus Kas Bulanan</h4>
                @if(count($charts['cashFlow']['labels'] ?? []) > 0 && (array_sum($charts['cashFlow']['income'] ?? []) > 0 || array_sum($charts['cashFlow']['expense'] ?? []) > 0))
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center gap-4 mt-3 text-xs">
                            <span class="flex items-center gap-1"><span class="w-2 h-2 bg-emerald-400 rounded-full"></span> Penerimaan</span>
                            <span class="flex items-center gap-1"><span class="w-2 h-2 bg-rose-400 rounded-full"></span> Pengeluaran</span>
                        </div>
                    </div>
                @else
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
                ...
                <div class="flex-grow flex flex-col items-center justify-center min-h-[200px] bg-slate-50 rounded-xl">
                    ...
                    @if(($kpi['overdue_invoices'] ?? 0) > 0)
                        <p class="text-rose-500 text-xs font-bold mt-2">{{ $kpi['overdue_invoices'] }} tagihan terlambat</p>
                    @endif
                </div>
            </div>
        </div>
