    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 md:p-8 border-b border-gray-100 bg-white flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-2xl font-extrabold text-gray-900">Detail Modul</h2>
                    <p class="text-sm font-medium text-gray-500 mt-1">Informasi lengkap data modul aplikasi.</p>
                </div>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('modules.edit', $module['Module_ID']) }}" class="inline-flex items-center justify-center px-4 py-2 border border-gray-200 rounded-xl shadow-sm text-sm font-bold text-gray-700 bg-white hover:bg-gray-50 transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
        ...
        <div class="p-6 md:p-8">
            <div class="flex items-center mb-8 pb-8 border-b border-gray-100">
                ...
                <div class="ml-6">
                    <h3 class="text-2xl font-extrabold text-gray-900">{{ $module['Module_Name'] ?? '-' }}</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div class="bg-gray-50 rounded-2xl p-6 border border-gray-100">
                    ...
                    <dl class="space-y-4">
                        <div>
                            <dt class="text-xs font-bold text-gray-500 uppercase">Grup Modul</dt>
                            <dd class="mt-1 text-sm font-semibold text-gray-900">{{ $module['Module_Group'] ?? '-' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-bold text-gray-500 uppercase">Urutan (Order)</dt>
                            <dd class="mt-1 text-sm font-semibold text-gray-900">
                                <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-gray-200 text-gray-800 font-bold text-sm">
                ...
                <div class="bg-gray-50 rounded-2xl p-6 border border-gray-100">
                    ...
                    <dl class="space-y-4">
                        <div>
                            <dt class="text-xs font-bold text-gray-500 uppercase">Dibuat Pada</dt>
                            <dd class="mt-1 text-sm font-semibold text-gray-900">{{ !empty($module['Created_At']) ? \Carbon\Carbon::parse($module['Created_At'])->format('d F Y, H:i:s') : '-' }}</dd>
                            <dd class="text-xs text-gray-500 mt-0.5">Oleh: {{ $module['Created_By'] ?? '-' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-bold text-gray-500 uppercase">Terakhir Diperbarui</dt>
                            <dd class="mt-1 text-sm font-semibold text-gray-900">{{ !empty($module['Updated_At']) ? \Carbon\Carbon::parse($module['Updated_At'])->format('d F Y, H:i:s') : '-' }}</dd>
                            <dd class="text-xs text-gray-500 mt-0.5">Oleh: {{ $module['Updated_By'] ?? '-' }}</dd>
                        </div>
                    </dl>
                </div>
                ...
                <div class="md:col-span-2 bg-gray-50 rounded-2xl p-6 border border-gray-100">
                    ...
                    <div class="text-sm text-gray-700 whitespace-pre-line bg-white p-4 rounded-xl border border-gray-200 shadow-sm min-h-[100px]">{{ $module['Notes'] ?: 'Tidak ada catatan.' }}</div>
                </div>
            </div>
        </div>
    </div>
