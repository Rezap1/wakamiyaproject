            ...
            <div class="space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div class="bg-slate-50 p-4 rounded-xl border border-slate-200">
                        ...
                        <p class="text-sm font-medium text-slate-800 mt-1">{{ !empty($program['Created_At']) ? \App\Helpers\DateHelper::format($program['Created_At'], 'd M Y, H:i') : '-' }}</p>
                        <p class="text-xs text-slate-500 mt-1">Oleh: {{ \App\Helpers\UserResolverHelper::getName($program['Created_By'] ?? '') }}</p>
                    </div>
                    <div class="bg-slate-50 p-4 rounded-xl border border-slate-200">
                        <p class="text-xs font-bold text-slate-400 uppercase">Terakhir Diperbarui</p>
                        <p class="text-sm font-medium text-slate-800 mt-1">{{ !empty($program['Updated_At']) ? \App\Helpers\DateHelper::format($program['Updated_At'], 'd M Y, H:i') : '-' }}</p>
