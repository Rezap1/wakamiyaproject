...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100 bg-white flex justify-between items-center">
            <div>
                <h2 class="text-xl font-extrabold text-gray-900">Perbarui Data Posisi</h2>
                <p class="text-sm font-medium text-gray-500 mt-1">ID Posisi: <span class="font-mono text-gray-900 bg-gray-100 px-2 py-0.5 rounded">{{ $position['Position_ID'] }}</span></p>
            </div>
        ...
        <div class="p-6 md:p-8">
            <form action="{{ route('positions.update', $position['Position_ID']) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="mt-2 relative rounded-xl shadow-sm">
                            <input type="text" name="Position_Code" id="Position_Code" value="{{ old('Position_Code', $position['Position_Code']) }}" 
                        ...
                        <div class="mt-2 relative rounded-xl shadow-sm">
                            <select name="Department_ID" id="Department_ID" 
                                ...
                                @foreach($departments as $dept)
                                    <option value="{{ $dept['Department_ID'] }}" {{ old('Department_ID', $position['Department_ID']) == $dept['Department_ID'] ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <div class="mt-2 relative rounded-xl shadow-sm">
                            <select name="Position_Level" id="Position_Level" 
                                ...
                                @php
                                    $levels = ['Direksi', 'Manajer', 'Supervisor', 'Staff'];
                                    $currentLevel = old('Position_Level', $position['Position_Level']);
                                @endphp
                                @foreach($levels as $level)
                                    <option value="{{ $level }}" {{ $currentLevel == $level ? 'selected' : '' }}>{{ $level }}</option>
                                @endforeach
                                @if(!in_array($currentLevel, $levels) && $currentLevel != '')
                                    <option value="{{ $currentLevel }}" selected>{{ $currentLevel }}</option>
                                @endif
                            </select>
                        </div>
                ...
                <!-- Status -->
                <div>
                    <label for="Is_Active" class="block text-sm font-bold text-gray-700">Status <span class="text-red-500">*</span></label>
                    <div class="mt-2 relative rounded-xl shadow-sm">
                        <select name="Is_Active" id="Is_Active" 
                            class="block w-full rounded-xl border-gray-300 focus:ring-primary-500 focus:border-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors @error('Is_Active') border-red-300 text-red-900 focus:ring-red-500 focus:border-red-500 @enderror">
                            <option value="TRUE" {{ old('Is_Active', $position['Is_Active'] ?? 'TRUE') === 'TRUE' ? 'selected' : '' }}>Aktif</option>
                            <option value="FALSE" {{ old('Is_Active', $position['Is_Active'] ?? 'TRUE') === 'FALSE' ? 'selected' : '' }}>Nonaktif</option>
                        </select>
                    </div>
                    @error('Is_Active')
                        <p class="mt-2 text-sm text-red-600 font-medium flex items-center">
                ...
                <div>
                    ...
                    <div class="mt-2">
                        <textarea id="Notes" name="Notes" rows="3" 
                            ...
                            placeholder="Tambahkan catatan khusus untuk posisi ini jika diperlukan...">{{ old('Notes', $position['Notes']) }}</textarea>
                    </div>
                </div>
                ...
                <div class="pt-6 border-t border-gray-100 flex justify-end gap-3">
                    ...
                    <button type="submit" id="submitBtn" class="px-6 py-3 border border-transparent shadow-lg text-sm font-bold rounded-xl text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-all hover:-translate-y-0.5 hover:shadow-primary-500/30 flex items-center justify-center gap-2">
                        <span id="submitText">Perbarui Posisi</span>
