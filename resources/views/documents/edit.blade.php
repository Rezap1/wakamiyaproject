    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 md:p-8 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-2xl font-extrabold text-gray-900">Edit Dokumen</h2>
                    <p class="text-sm font-medium text-gray-500 mt-1">Ubah data atau perbarui status verifikasi untuk dokumen <span class="font-bold text-gray-700">{{ $document['Document_Number'] ?? $document['Document_ID'] }}</span>.</p>
                </div>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('documents.show', $document['Document_ID']) }}" class="inline-flex items-center justify-center px-4 py-2 border border-gray-200 rounded-xl shadow-sm text-sm font-bold text-gray-700 bg-white hover:bg-gray-50 transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
        ...
        <div class="p-6 md:p-8">
            <form action="{{ route('documents.update', $document['Document_ID']) }}" method="POST" class="space-y-10">
                @csrf
                @method('PUT')
                        ...
                        <div>
                            ...
                            <select name="Student_ID" id="Student_ID" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" required>
                                ...
                                @foreach($students as $student)
                                    <option value="{{ $student['Student_ID'] }}" {{ old('Student_ID', $document['Student_ID'] ?? '') == $student['Student_ID'] ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="Application_ID" id="Application_ID" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">
                                ...
                                @foreach($applications as $app)
                                    <option value="{{ $app['Application_ID'] }}" {{ old('Application_ID', $document['Application_ID'] ?? '') == $app['Application_ID'] ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="Document_Number" id="Document_Number" value="{{ old('Document_Number', $document['Document_Number'] ?? '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">
                        ...
                        <div>
                            ...
                            <select name="Document_Type" id="Document_Type" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" required>
                                ...
                                <option value="CONTRACT" {{ old('Document_Type', $document['Document_Type'] ?? '') == 'CONTRACT' ? 'selected' : '' }}>Kontrak Kerja</option>
                                <option value="MEDICAL" {{ old('Document_Type', $document['Document_Type'] ?? '') == 'MEDICAL' ? 'selected' : '' }}>Medical Checkup</option>
                                <option value="CERTIFICATE" {{ old('Document_Type', $document['Document_Type'] ?? '') == 'CERTIFICATE' ? 'selected' : '' }}>Sertifikat/Ijazah</option>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="Document_Name" id="Document_Name" value="{{ old('Document_Name', $document['Document_Name'] ?? '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" required>
                        ...
                        <div>
                            ...
                            <input type="text" name="File_Name" id="File_Name" value="{{ old('File_Name', $document['File_Name'] ?? '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">
                        ...
                        <div>
                            ...
                            <input type="url" name="File_URL" id="File_URL" value="{{ old('File_URL', $document['File_URL'] ?? '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="date" name="Issue_Date" id="Issue_Date" value="{{ old('Issue_Date', !empty($document['Issue_Date']) ? \Carbon\Carbon::parse($document['Issue_Date'])->format('Y-m-d') : '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">
                        ...
                        <div>
                            ...
                            <input type="date" name="Expiry_Date" id="Expiry_Date" value="{{ old('Expiry_Date', !empty($document['Expiry_Date']) ? \Carbon\Carbon::parse($document['Expiry_Date'])->format('Y-m-d') : '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">
                        ...
                        <div>
                            ...
                            <select name="Document_Status" id="Document_Status" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" required>
                                <option value="PENDING" {{ old('Document_Status', $document['Document_Status'] ?? '') == 'PENDING' ? 'selected' : '' }}>Pending (Menunggu Verifikasi)</option>
                                <option value="VERIFIED" {{ old('Document_Status', $document['Document_Status'] ?? '') == 'VERIFIED' ? 'selected' : '' }}>Verified (Disetujui)</option>
                                <option value="REJECTED" {{ old('Document_Status', $document['Document_Status'] ?? '') == 'REJECTED' ? 'selected' : '' }}>Rejected (Ditolak)</option>
                                <option value="EXPIRED" {{ old('Document_Status', $document['Document_Status'] ?? '') == 'EXPIRED' ? 'selected' : '' }}>Expired (Kedaluwarsa)</option>
                            </select>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <textarea name="Remarks" id="Remarks" rows="2" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">{{ old('Remarks', $document['Remarks'] ?? '') }}</textarea>
                        </div>

                        <div>
                            <label for="Notes" class="block text-sm font-bold text-gray-700 mb-2">Catatan Internal WMS</label>
                            <textarea name="Notes" id="Notes" rows="2" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">{{ old('Notes', $document['Notes'] ?? '') }}</textarea>
                        </div>
                    </div>
                </div>
