...
<div class="max-w-4xl mx-auto space-y-6" x-data="scoreEditForm(@js($assessmentConfigs ?? []), @js($category), @js($details))">
    ...
    <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <form x-ref="form" action="{{ route('scores.update', $score['Score_ID'] ?? '') }}" method="POST" class="space-y-6 p-4 sm:p-6 md:p-8">
            ...
            <div x-show="activeConfig && activeAspects.length === 0" class="border-t border-slate-100 pt-5"><label class="mb-1 block text-sm font-bold text-slate-700"><span x-text="numericLabel">Nilai Ujian Bab</span> <span class="text-rose-600">*</span></label><input name="Score_Value" type="number" min="0" max="100" step="0.01" value="{{ old('Score_Value', old('Score', $score['Score_Value'] ?? ($score['Score'] ?? ''))) }}" :required="!!activeConfig && activeAspects.length === 0" class="w-full rounded-xl border-slate-200 bg-slate-50 text-lg font-bold"></div>
