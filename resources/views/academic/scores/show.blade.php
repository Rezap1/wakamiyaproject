...
<div class="max-w-5xl mx-auto space-y-6">
    <x-universal.detail-layout title="{{ $score['Student_Name'] ?? ($score['Student_ID'] ?? 'Siswa') }}" description="Assessment: {{ $score['Assessment_Title'] ?? ($score['Assessment_ID'] ?? '-') }} | ID Nilai: {{ $score['Score_ID'] ?? '-' }}" status="{{ $statusText }}" badgeColor="{{ $result['pass'] ? 'green' : 'red' }}" :breadcrumbs="['Dasbor' => route('dashboard'), 'Nilai' => route('scores.index'), 'Detail' => '#']">
        ...
        <x-slot:information>
            <div class="space-y-8">
                <div class="rounded-2xl bg-gradient-to-r from-slate-900 to-slate-800 p-6 text-white"><span class="text-xs font-bold uppercase tracking-wider text-slate-400">Kategori Assessment</span><h2 class="mt-1 text-xl font-bold">{{ $categoryLabel }}</h2><p class="mt-1 text-xs text-slate-400">Siswa ID: {{ $score['Student_ID'] ?? '-' }} &bull; Tanggal: {{ !empty($score['Created_At']) ? \Carbon\Carbon::parse($score['Created_At'])->format('d M Y') : '-' }}</p><p class="mt-4 text-4xl font-black">{{ $rawScore }} <span class="text-sm font-normal text-slate-400">/ 100</span></p></div>
                <div>
        ...
        <x-slot:audit><div class="space-y-4"><h3 class="border-b border-slate-100 pb-2 text-xs font-bold uppercase tracking-wider text-slate-400">System Metadata & Audit Trail</h3><div class="grid grid-cols-1 gap-4 sm:grid-cols-2"><div><p class="text-xs font-bold text-slate-400">ID Rekaman Nilai</p><p class="font-mono font-bold">{{ $score['Score_ID'] ?? '-' }}</p></div><div><p class="text-xs font-bold text-slate-400">ID Assessment</p><p class="font-mono font-bold">{{ $score['Assessment_ID'] ?? ($score['Assignment_ID'] ?? '-') }}</p></div><div><p class="text-xs font-bold text-slate-400">Waktu Pencatatan</p><p>{{ $score['Created_At'] ?? '-' }}</p></div><div><p class="text-xs font-bold text-slate-400">Waktu Diperbarui</p><p>{{ $score['Updated_At'] ?? '-' }}</p></div></div></div></x-slot:audit>
