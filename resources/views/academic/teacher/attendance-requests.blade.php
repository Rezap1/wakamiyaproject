...
<div class="space-y-6">
    <x-page-header 
        ...
        description="Verifikasi pengajuan izin/sakit dari siswa di kelas yang Anda ajar."
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="p-5 border-b border-slate-100 bg-slate-50">
            ...
            <p class="text-sm text-slate-500 mt-1">Setujui atau tolak hanya pengajuan yang berada dalam scope kelas dan jadwal Anda.</p>
        </div>
            ...
            @forelse($attendanceRequests as $req)
                <div class="p-4 bg-white space-y-2">
                    ...
                    <div class="text-xs text-slate-500">
                        ...
                        <p><span class="font-semibold">Target:</span> {{ $req['Target_Display'] ?? 'Target tidak tersedia' }}</p>
                        <p class="mt-1 italic">"{{ $req['Reason'] ?? 'Tidak ada alasan' }}"</p>
                    </div>
                    <div class="flex gap-2">
                        <a href="{{ route('teacher.workspace.attendance-requests.show', $req['Request_ID']) }}" class="inline-flex flex-1 items-center justify-center rounded-lg bg-sky-50 px-3 py-2 text-xs font-bold text-sky-700 transition-colors hover:bg-sky-100">
                            Detail
                        </a>
                        @if(!empty($req['Evidence_URL']))
                            <a href="{{ route('teacher.workspace.attendance-requests.evidence', $req['Request_ID']) }}" class="inline-flex flex-1 items-center justify-center rounded-lg bg-blue-50 px-3 py-2 text-xs font-bold text-blue-700 transition-colors hover:bg-blue-100">
                                Bukti
                            </a>
                        @endif
                    </div>
            ...
            <table class="w-full text-left text-sm whitespace-nowrap">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="px-6 py-4 font-semibold text-slate-600">{{ $req['Class_Name'] ?? 'Kelas tidak ditemukan' }}</td>
                        <td class="px-6 py-4 font-bold text-slate-900">{{ $req['Student_Name'] }}</td>
                        <td class="px-6 py-4 font-semibold text-blue-600">{{ $req['Request_Type_Label'] ?? $req['Request_Type'] ?? '-' }}</td>
                        <td class="px-6 py-4 text-slate-600 max-w-sm truncate">{{ $req['Target_Display'] ?? 'Target tidak tersedia' }}</td>
                        ...
                        <td class="px-6 py-4 text-right">
                            @if(!empty($req['Evidence_URL']))
                                <a href="{{ route('teacher.workspace.attendance-requests.evidence', $req['Request_ID']) }}" class="inline-block bg-blue-50 text-blue-700 hover:bg-blue-100 font-bold text-xs px-3 py-1.5 rounded-lg border border-blue-200 transition-colors mr-1">
                                    Bukti
                                </a>
                            @endif
                            <a href="{{ route('teacher.workspace.attendance-requests.show', $req['Request_ID']) }}" class="inline-block bg-sky-50 text-sky-700 hover:bg-sky-100 font-bold text-xs px-3 py-1.5 rounded-lg border border-sky-200 transition-colors">
                                Detail
                            </a>
                        </td>
                    </tr>
