                    ...
                    <div class="border border-slate-200 rounded-xl p-4 hover:shadow-md transition-shadow">
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                ...
                                <p class="text-sm text-slate-600 mt-1 whitespace-pre-line break-words">{{ $ann['Message'] ?? $ann['Content'] ?? $ann['Description'] ?? '' }}</p>
                                <div class="flex items-center gap-3 mt-2">
                                    <span class="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold text-slate-600">{{ $ann['Priority_Label'] ?? 'Informasi' }}</span>
                                    @if(!empty($ann['Expiry_Label']))
                                    <span class="text-xs text-slate-400">Berakhir: {{ $ann['Expiry_Label'] }}</span>
                                    @elseif(!empty($ann['Expires_At']) || !empty($ann['Expired_Date']))
                            ...
                            @if(Route::has('student.portal.announcements.show') && !empty($ann['Announcement_ID']))
                            <a href="{{ route('student.portal.announcements.show', $ann['Announcement_ID']) }}" class="shrink-0 ml-3 px-3 py-1.5 text-xs font-bold bg-emerald-100 text-emerald-700 rounded-lg hover:bg-emerald-200 transition-colors">Lihat Detail</a>
                            @elseif(!empty($ann['Attachment_URL']))
