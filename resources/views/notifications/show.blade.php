...
<div class="max-w-3xl mx-auto space-y-6">
    <x-page-header title="Detail Notifikasi" description="Lihat detail pesan." :breadcrumbs="['Dasbor' => route('dashboard.administrator'), 'Notifikasi' => route('notifications.index'), 'Detail' => '#']" />
    ...
    <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
        ...
        <div class="mt-12 pt-6 border-t border-slate-100 flex gap-4">
            ...
            <form action="{{ route('notifications.destroy', $notification['Notification_ID']) }}" method="POST">
                @csrf
                @method('DELETE')
                <button type="submit" class="px-4 py-2 text-sm font-bold text-rose-600 bg-rose-50 rounded-lg hover:bg-rose-100 transition-colors" onsubmit="return confirm('Hapus notifikasi ini?');">Hapus</button>
            </form>
        </div>
    </div>
</div>
