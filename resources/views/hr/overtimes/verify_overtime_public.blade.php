@extends('layouts.app')
@section('header', 'Verifikasi Surat Lembur')
@section('content')
...
@php
    ...
    $documentNumber = $overtime['Document_Number'] ?? 'Dokumen Terverifikasi';
...
<div class="max-w-xl mx-auto py-8 px-4">
    <div class="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm text-center space-y-5">
        ...
        <div>
            ...
            <h2 class="text-2xl font-black text-slate-800 mt-3">Surat Lembur Terverifikasi</h2>
            <p class="text-xs text-slate-500 mt-1">Detail upah dan metadata HR internal disembunyikan dari halaman publik.</p>
        </div>
        ...
        <div class="space-y-3 pt-4 text-left border-t border-slate-100 text-sm">
            ...
            <div class="flex justify-between gap-4">
                ...
                <span class="font-bold text-slate-800 text-right">{{ $maskName($employee['Full_Name'] ?? $overtime['Employee_Name'] ?? '') }}</span>
            </div>
            <div class="flex justify-between gap-4">
                <span class="text-slate-500 font-medium">Tanggal</span>
                <span class="font-bold text-slate-800 text-right">{{ $overtime['Date'] ?? '-' }}</span>
            </div>
            <div class="flex justify-between gap-4">
