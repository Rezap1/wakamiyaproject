...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100 bg-white flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            <div>
                <h2 class="text-xl font-extrabold text-gray-900">Formulir Pembaruan Kelas (Rombel)</h2>
                <p class="text-sm font-medium text-gray-500 mt-1">ID: <span class="font-bold text-primary-600 bg-primary-50 px-2 py-0.5 rounded font-mono">{{ $class['Class_ID'] }}</span></p>
            </div>
            <div>
        ...
        <div class="p-6 md:p-8">
            <form action="{{ route('classes.update', $class['Class_ID']) }}" method="POST" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="Class_Code" id="Class_Code" value="{{ old('Class_Code', $class['Class_Code']) }}" class="mt-2 block w-full rounded-xl border-gray-300 focus:ring-primary-500 focus:border-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors uppercase @error('Class_Code') border-red-300 text-red-900 @enderror">
                    ...
                    <div>
                        ...
                        <input type="text" name="Class_Name" id="Class_Name" value="{{ old('Class_Name', $class['Class_Name']) }}" class="mt-2 block w-full rounded-xl border-gray-300 focus:ring-primary-500 focus:border-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors @error('Class_Name') border-red-300 text-red-900 @enderror">
                    ...
                    <div>
                        ...
                        <select name="Program_ID" id="Program_ID" class="mt-2 block w-full rounded-xl border-gray-300 focus:ring-primary-500 focus:border-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors @error('Program_ID') border-red-300 text-red-900 @enderror">
                            ...
                            @foreach($programs as $program)
                                <option value="{{ $program['Program_ID'] }}" {{ old('Program_ID', $class['Program_ID']) == $program['Program_ID'] ? 'selected' : '' }}>
                                    {{ $program['Program_Code'] }} - {{ $program['Program_Name'] }} {{ ($program['Is_Active'] ?? 'TRUE') === 'FALSE' ? '(Nonaktif)' : '' }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="Homeroom_Teacher_ID" id="Homeroom_Teacher_ID" class="mt-2 block w-full rounded-xl border-gray-300 focus:ring-primary-500 focus:border-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors @error('Homeroom_Teacher_ID') border-red-300 text-red-900 @enderror">
                            ...
                            @foreach($teachers as $teacher)
                                <option value="{{ $teacher['Teacher_ID'] }}" {{ old('Homeroom_Teacher_ID', $class['Homeroom_Teacher_ID']) == $teacher['Teacher_ID'] ? 'selected' : '' }}>
                                    {{ $teacher['Teacher_Code'] }} - {{ $teacher['Full_Name'] }} ({{ $teacher['Specialization'] }}) {{ ($teacher['Is_Active'] ?? 'TRUE') === 'FALSE' ? '(Nonaktif)' : '' }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <div class="mt-2 relative rounded-xl shadow-sm">
                            ...
                            <input type="number" name="Capacity" id="Capacity" min="1" value="{{ old('Capacity', $class['Capacity']) }}" class="block w-full pl-11 rounded-xl border-gray-300 focus:ring-primary-500 focus:border-primary-500 sm:text-sm py-3 bg-gray-50 focus:bg-white transition-colors @error('Capacity') border-red-300 text-red-900 @enderror">
                    ...
                    <div>
                        ...
                        <div class="mt-2 relative rounded-xl shadow-sm">
                            ...
                            <input type="number" name="Current_Student" id="Current_Student" min="0" value="{{ old('Current_Student', $class['Current_Student']) }}" class="block w-full pl-11 rounded-xl border-gray-300 focus:ring-primary-500 focus:border-primary-500 sm:text-sm py-3 bg-gray-50 focus:bg-white transition-colors @error('Current_Student') border-red-300 text-red-900 @enderror">
                            <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                                <span class="text-gray-500 sm:text-sm font-medium">Siswa</span>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <select name="Class_Status" id="Class_Status" class="mt-2 block w-full rounded-xl border-gray-300 focus:ring-primary-500 focus:border-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors @error('Class_Status') border-red-300 text-red-900 @enderror">
                            <option value="Persiapan" {{ old('Class_Status', $class['Class_Status']) == 'Persiapan' ? 'selected' : '' }}>Persiapan</option>
                            <option value="Aktif" {{ old('Class_Status', $class['Class_Status']) == 'Aktif' ? 'selected' : '' }}>Aktif Berjalan</option>
                            <option value="Penuh" {{ old('Class_Status', $class['Class_Status']) == 'Penuh' ? 'selected' : '' }}>Penuh</option>
                            <option value="Selesai" {{ old('Class_Status', $class['Class_Status']) == 'Selesai' ? 'selected' : '' }}>Selesai (Lulus)</option>
                            <option value="Ditutup" {{ old('Class_Status', $class['Class_Status']) == 'Ditutup' ? 'selected' : '' }}>Ditutup / Nonaktif</option>
                        </select>
                    ...
                    <div>
                        ...
                        <select name="Is_Active" id="Is_Active" class="mt-2 block w-full rounded-xl border-gray-300 focus:ring-primary-500 focus:border-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors @error('Is_Active') border-red-300 text-red-900 @enderror">
                            <option value="TRUE" {{ old('Is_Active', $class['Is_Active'] ?? 'TRUE') === 'TRUE' ? 'selected' : '' }}>Aktif</option>
                            <option value="FALSE" {{ old('Is_Active', $class['Is_Active'] ?? 'TRUE') === 'FALSE' ? 'selected' : '' }}>Nonaktif (Soft Delete)</option>
                        </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="Description" name="Description" rows="2" class="mt-2 block w-full rounded-xl border-gray-300 focus:ring-primary-500 focus:border-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">{{ old('Description', $class['Description']) }}</textarea>
                    </div>

                    <!-- Notes -->
                    <div class="md:col-span-2">
                        <label for="Notes" class="block text-sm font-bold text-gray-700">Catatan Internal</label>
                        <textarea id="Notes" name="Notes" rows="2" class="mt-2 block w-full rounded-xl border-gray-300 focus:ring-primary-500 focus:border-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">{{ old('Notes', $class['Notes']) }}</textarea>
                    </div>
                </div>
                ...
                <div class="pt-6 border-t border-gray-100 flex justify-end gap-3 sticky bottom-0 bg-white/80 backdrop-blur-md pb-4 rounded-b-2xl">
                    ...
                    <button type="submit" id="submitBtn" class="px-6 py-3 border border-transparent shadow-lg text-sm font-bold rounded-xl text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-all hover:-translate-y-0.5 hover:shadow-primary-500/30 flex items-center justify-center gap-2">
                        <span id="submitText">Perbarui Kelas</span>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function filterBatches() {
            ...
            if (selectedProgramId) {
                // If the selected option is now disabled, reset value
