...
<div id="{{ $id }}" class="fixed inset-0 z-50 overflow-y-auto {{ $show ? 'flex' : 'hidden' }} items-center justify-center p-4" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    ...
    <!-- Modal Panel -->
    <div class="relative bg-white rounded-2xl shadow-xl border border-slate-200 transform transition-all opacity-0 scale-95 modal-panel w-full {{ $maxWidthClass }} mx-auto flex flex-col max-h-[90vh]">
        ...
        <!-- Footer -->
        @if(isset($footer))
        <div class="px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl flex justify-end gap-3 shrink-0">
            {{ $footer }}
        </div>
        @endif
    </div>
