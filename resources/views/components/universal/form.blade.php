...
<div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
    ...
    <form action="{{ $action }}" method="{{ $method === 'GET' ? 'GET' : 'POST' }}" {{ $hasFiles ? 'enctype="multipart/form-data"' : '' }} x-data="smartForm()" @submit="isSubmitting = true">
        ...
        <div class="p-6 border-t border-slate-100 bg-slate-50 flex items-center justify-end gap-3">
            <button type="button" onclick="history.back()" class="px-5 py-2.5 text-sm font-bold text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors shadow-sm" :disabled="isSubmitting">
                Batal
            </button>
            <button type="submit" class="px-5 py-2.5 text-sm font-bold text-white bg-blue-600 rounded-xl hover:bg-blue-700 transition-colors shadow-sm shadow-blue-600/20 flex items-center gap-2" :disabled="isSubmitting" :class="{'opacity-75 cursor-not-allowed': isSubmitting}">
