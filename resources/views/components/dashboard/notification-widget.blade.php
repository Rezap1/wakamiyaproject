    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
        ...
        <div class="space-y-4">
            @forelse($recentNotifs as $notif)
                ...
                @endphp
                <div class="flex items-start gap-4 p-4 rounded-xl {{ $isRead ? 'bg-slate-50' : 'bg-emerald-50/50 border border-blue-100' }}">
                    <div class="flex-grow">
                        ...
                        <a href="{{ route('notifications.show', $notif['Notification_ID']) }}" class="font-bold text-slate-800 text-sm hover:text-emerald-600 transition-colors {{ $isRead ? '' : 'text-blue-900' }}">{{ $notif['Title'] ?? 'Notification' }}</a>
            ...
            @empty
                <div class="p-8 text-center text-slate-400 bg-slate-50 rounded-xl border border-slate-200 border-dashed">
