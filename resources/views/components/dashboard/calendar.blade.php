<x-card class="p-6 lg:p-8 flex flex-col h-full hover:shadow-md transition-shadow duration-300">
    ...
    <div class="grid grid-cols-7 gap-1 text-center">
        ...
        @for($i = 1; $i <= $daysInMonth; $i++)
            ...
            @endphp
            <div class="relative py-2 text-[12px] font-bold rounded-lg cursor-pointer transition-all {{ $isToday ? 'bg-emerald-600 text-white shadow-md shadow-blue-500/30' : 'text-slate-600 hover:bg-slate-50' }}">
                {{ $i }}
                @if($hasEvent && !$isToday)
    ...
    <div class="mt-6 border-t border-slate-100 pt-6 space-y-4 flex-1">
        ...
        <div class="flex items-start gap-3 group cursor-pointer">
            ...
            <div>
                <p class="text-[12px] font-bold text-slate-800 group-hover:text-emerald-600 transition-colors">Evaluasi Interview Peserta</p>
