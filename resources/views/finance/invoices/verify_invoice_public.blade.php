@extends('layouts.app')
@section('header', 'Verifikasi Invoice')
@section('content')
...
@php
    ...
    $documentNumber = $invoice['Document_Number'] ?? $invoice['Invoice_Number'] ?? 'Dokumen Terverifikasi';
    ...
    <div class="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm text-center space-y-5">
        ...
        <div class="space-y-3 pt-4 text-left border-t border-slate-100 text-sm">
            ...
            <div class="flex justify-between gap-4">
                ...
                <span class="font-bold text-slate-800 text-right">{{ $invoice['Category'] ?? '-' }}</span>
            </div>
            <div class="flex justify-between gap-4">
