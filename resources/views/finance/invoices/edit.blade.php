...
<div class="space-y-6" x-data="invoiceForm()">
    <x-page-header title="Edit Tagihan" description="Perbarui tagihan." :breadcrumbs="['Dasbor' => route('dashboard.finance'), 'Tagihan' => route('invoices.index'), 'Edit' => '#']" />
    ...
    <form action="{{ route('invoices.update', $invoice['Invoice_ID']) }}" method="POST">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-slate-50">
                ...
                <p class="text-xs text-slate-500 mt-1">Perbarui detail untuk tagihan.</p>
            </div>
            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-4">
                    <x-input type="number" name="Amount" label="Nominal (IDR)" value="{{ $invoice['Amount'] ?? '' }}" required />
                    <x-input type="date" name="Due_Date" label="Jatuh Tempo" value="{{ $invoice['Due_Date'] ?? date('Y-m-d', strtotime('+7 days')) }}" />
                    <div>
                ...
                <div class="flex justify-end gap-3 mt-8 border-t border-slate-100 pt-6">
                    ...
                    <button type="submit" :disabled="!student.Student_ID && isSelected && '{{ $invoice['Invoice_Type'] ?? 'STUDENT' }}' === 'STUDENT'" class="px-6 py-2.5 text-sm font-bold text-white bg-emerald-600 rounded-xl shadow-sm hover:bg-emerald-700 focus:ring-4 focus:ring-emerald-200 transition-all disabled:opacity-50 disabled:cursor-not-allowed">Perbarui Tagihan</button>
                </div>
            </div>
        </div>
    </form>
