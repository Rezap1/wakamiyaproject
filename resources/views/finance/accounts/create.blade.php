...
@section('header')
        <h2 class="font-semibold text-xl text-slate-800 leading-tight">Tambah Akun Master</h2>
@endsection
            ...
            <div class="bg-white p-6 rounded shadow max-w-2xl">
                <form action="{{ route('accounts.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="Account_Code" value="{{ old('Account_Code') }}" class="form-input w-full rounded-md border-gray-300" required>
                    </div>

                    <div class="mb-4">
                        <label class="block text-gray-700 font-bold mb-2">Nama Akun *</label>
                        <input type="text" name="Account_Name" value="{{ old('Account_Name') }}" class="form-input w-full rounded-md border-gray-300" required>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="Description" class="form-textarea w-full rounded-md border-gray-300">{{ old('Description') }}</textarea>
                    </div>

                    <div class="flex justify-end gap-2">
                        <a href="{{ route('accounts.index') }}" class="px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300">Batal</a>
                        <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Simpan Akun</button>
                    </div>
                </form>
            </div>
        </div>
