@extends('layouts.app')
@section('header', 'Verifikasi Kuitansi')
@section('content')
...
@php
    ...
    $documentNumber = $payment['Receipt_Number'] ?? 'Dokumen Terverifikasi';
...
<div class="max-w-xl mx-auto py-8 px-4">
    <div class="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm text-center space-y-5">
        ...
        <div>
            ...
            <h2 class="text-2xl font-black text-slate-800 mt-3">Kuitansi Terverifikasi</h2>
            <p class="text-xs text-slate-500 mt-1">Nominal, metode pembayaran, dan referensi internal disembunyikan dari halaman publik.</p>
        </div>
        ...
        <div class="space-y-3 pt-4 text-left border-t border-slate-100 text-sm">
            ...
            <div class="flex justify-between gap-4">
                <span class="text-slate-500 font-medium">Penerima</span>
                <span class="font-bold text-slate-800 text-right">{{ $maskName($customer['name'] ?? '') }}</span>
            </div>
            <div class="flex justify-between gap-4">
