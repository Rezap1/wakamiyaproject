...
@section('header', 'Edit Data Matching')
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 md:p-8 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-2xl font-extrabold text-gray-900">Edit Data Matching</h2>
                    <p class="text-sm font-medium text-gray-500 mt-1">Ubah status, tanggal, atau catatan untuk matching <span class="font-bold text-gray-700">{{ $matching['Matching_Number'] ?? $matching['Matching_ID'] }}</span>.</p>
                </div>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('matchings.show', $matching['Matching_ID']) }}" class="inline-flex items-center justify-center px-4 py-2 border border-gray-200 rounded-xl shadow-sm text-sm font-bold text-gray-700 bg-white hover:bg-gray-50 transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
        ...
        <div class="p-6 md:p-8">
            <form action="{{ route('matchings.update', $matching['Matching_ID']) }}" method="POST" class="space-y-10">
                @csrf
                @method('PUT')
                        ...
                        <div>
                            ...
                            <select name="Student_ID" id="Student_ID" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" required>
                                ...
                                @foreach($students as $student)
                                    <option value="{{ $student['Student_ID'] }}" {{ old('Student_ID', $matching['Student_ID'] ?? '') == $student['Student_ID'] ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="Job_Order_ID" id="Job_Order_ID" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" required>
                                ...
                                @foreach($jobOrders as $jo)
                                    <option value="{{ $jo['Job_Order_ID'] }}" {{ old('Job_Order_ID', $matching['Job_Order_ID'] ?? '') == $jo['Job_Order_ID'] ? 'selected' : '' }}>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select name="Interview_ID" id="Interview_ID" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">
                                ...
                                @foreach($interviews as $interview)
                                    <option value="{{ $interview['Interview_ID'] }}" {{ old('Interview_ID', $matching['Interview_ID'] ?? '') == $interview['Interview_ID'] ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div class="lg:col-span-2">
                            ...
                            <input type="text" name="Matching_Number" id="Matching_Number" value="{{ old('Matching_Number', $matching['Matching_Number'] ?? '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" placeholder="Cth: MTC-2023-001">
                        ...
                        <div>
                            ...
                            <input type="date" name="Matching_Date" id="Matching_Date" value="{{ old('Matching_Date', !empty($matching['Matching_Date']) ? \Carbon\Carbon::parse($matching['Matching_Date'])->format('Y-m-d') : '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" required>
                        ...
                        <div>
                            ...
                            <select name="Matching_Status" id="Matching_Status" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" required>
                                <option value="PROPOSED" {{ old('Matching_Status', $matching['Matching_Status'] ?? '') == 'PROPOSED' ? 'selected' : '' }}>Proposed (Diajukan)</option>
                                <option value="REVIEWING" {{ old('Matching_Status', $matching['Matching_Status'] ?? '') == 'REVIEWING' ? 'selected' : '' }}>Reviewing (Ditinjau)</option>
                                <option value="ACCEPTED" {{ old('Matching_Status', $matching['Matching_Status'] ?? '') == 'ACCEPTED' ? 'selected' : '' }}>Accepted (Diterima)</option>
                                <option value="REJECTED" {{ old('Matching_Status', $matching['Matching_Status'] ?? '') == 'REJECTED' ? 'selected' : '' }}>Rejected (Ditolak)</option>
                                <option value="WITHDRAWN" {{ old('Matching_Status', $matching['Matching_Status'] ?? '') == 'WITHDRAWN' ? 'selected' : '' }}>Withdrawn (Ditarik)</option>
                            </select>
                        ...
                        <div class="lg:col-span-2">
                            ...
                            <input type="date" name="Company_Approval_Date" id="Company_Approval_Date" value="{{ old('Company_Approval_Date', !empty($matching['Company_Approval_Date']) ? \Carbon\Carbon::parse($matching['Company_Approval_Date'])->format('Y-m-d') : '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">
                        </div>

                        <div class="lg:col-span-2">
                            <label for="Student_Approval_Date" class="block text-sm font-bold text-gray-700 mb-2">Tanggal Persetujuan Kandidat</label>
                            <input type="date" name="Student_Approval_Date" id="Student_Approval_Date" value="{{ old('Student_Approval_Date', !empty($matching['Student_Approval_Date']) ? \Carbon\Carbon::parse($matching['Student_Approval_Date'])->format('Y-m-d') : '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <textarea name="Remarks" id="Remarks" rows="3" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">{{ old('Remarks', $matching['Remarks'] ?? '') }}</textarea>
                        </div>

                        <div>
                            <label for="Notes" class="block text-sm font-bold text-gray-700 mb-2">Catatan Internal WMS</label>
                            <textarea name="Notes" id="Notes" rows="2" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">{{ old('Notes', $matching['Notes'] ?? '') }}</textarea>
                        </div>
                    </div>
                </div>
