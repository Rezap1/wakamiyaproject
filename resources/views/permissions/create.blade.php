...
@section('header', 'Konfigurasi Hak Akses Baru')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100 bg-white">
            <h2 class="text-xl font-extrabold text-gray-900">Tambah Aturan Hak Akses</h2>
            <p class="text-sm font-medium text-gray-500 mt-1">Tentukan hak akses untuk *Role* tertentu pada suatu Modul.</p>
        </div>
            ...
            <form action="{{ route('permissions.store') }}" method="POST" class="space-y-8">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Pilih Modul Sistem <span class="text-red-500">*</span></label>
                            <p class="text-xs text-gray-500 mb-3">Anda dapat memilih lebih dari satu modul untuk menerapkan matriks akses yang sama sekaligus.</p>
                            <div class="space-y-2 max-h-48 overflow-y-auto pr-2 bg-gray-50 p-4 rounded-xl border border-gray-200">
                                @foreach($modules as $module)
                                <div class="flex items-center">
                                    <input id="module_{{ $module['Module_ID'] }}" name="Module_ID[]" type="checkbox" value="{{ $module['Module_ID'] }}" class="w-4 h-4 text-primary-600 bg-white border-gray-300 rounded focus:ring-primary-500" {{ is_array(old('Module_ID')) && in_array($module['Module_ID'], old('Module_ID')) ? 'checked' : '' }}>
                                    <label for="module_{{ $module['Module_ID'] }}" class="ml-2 text-sm font-medium text-gray-700 cursor-pointer">
                    ...
                    <div class="bg-gray-50 p-6 rounded-2xl border border-gray-200 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                        ...
                        <div class="relative flex items-start p-4 border border-gray-200 bg-white rounded-xl shadow-sm hover:border-{{ $config['color'] }}-300 transition-colors group">
                            <div class="flex items-center h-5 mt-1">
                                <input id="{{ $field }}" name="{{ $field }}" type="checkbox" value="1" {{ old($field) ? 'checked' : '' }} class="focus:ring-{{ $config['color'] }}-500 h-5 w-5 text-{{ $config['color'] }}-600 border-gray-300 rounded cursor-pointer transition-all">
                            </div>
