    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 md:p-8 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-2xl font-extrabold text-gray-900">Edit Data Visa</h2>
                    <p class="text-sm font-medium text-gray-500 mt-1">Perbarui data atau ubah status pengajuan Visa <span class="font-bold text-gray-700">{{ $visa['Visa_Number'] ?? $visa['Visa_ID'] }}</span>.</p>
                </div>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('visas.show', $visa['Visa_ID']) }}" class="inline-flex items-center justify-center px-4 py-2 border border-gray-200 rounded-xl shadow-sm text-sm font-bold text-gray-700 bg-white hover:bg-gray-50 transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
        ...
        <div class="p-6 md:p-8">
            <form action="{{ route('visas.update', $visa['Visa_ID']) }}" method="POST" class="space-y-10">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="Student_ID" id="Student_ID" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" required>
                                ...
                                @foreach($students as $student)
                                    <option value="{{ $student['Student_ID'] }}" {{ old('Student_ID', $visa['Student_ID'] ?? '') == $student['Student_ID'] ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="text" name="Passport_Number" id="Passport_Number" value="{{ old('Passport_Number', $visa['Passport_Number'] ?? '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" required>
                        ...
                        <div>
                            ...
                            <select name="COE_ID" id="COE_ID" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">
                                ...
                                @foreach($coes as $coe)
                                    <option value="{{ $coe['COE_ID'] }}" {{ old('COE_ID', $visa['COE_ID'] ?? '') == $coe['COE_ID'] ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="Application_ID" id="Application_ID" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">
                                ...
                                @foreach($applications as $app)
                                    <option value="{{ $app['Application_ID'] }}" {{ old('Application_ID', $visa['Application_ID'] ?? '') == $app['Application_ID'] ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="text" name="Visa_Number" id="Visa_Number" value="{{ old('Visa_Number', $visa['Visa_Number'] ?? '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" required>
                        ...
                        <div>
                            ...
                            <input type="text" name="Visa_Type" id="Visa_Type" value="{{ old('Visa_Type', $visa['Visa_Type'] ?? '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" required>
                        ...
                        <div>
                            ...
                            <input type="text" name="Embassy" id="Embassy" value="{{ old('Embassy', $visa['Embassy'] ?? '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" required>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div>
                            ...
                            <input type="date" name="Submission_Date" id="Submission_Date" value="{{ old('Submission_Date', !empty($visa['Submission_Date']) ? \Carbon\Carbon::parse($visa['Submission_Date'])->format('Y-m-d') : '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">
                        ...
                        <div>
                            ...
                            <input type="date" name="Approval_Date" id="Approval_Date" value="{{ old('Approval_Date', !empty($visa['Approval_Date']) ? \Carbon\Carbon::parse($visa['Approval_Date'])->format('Y-m-d') : '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">
                        ...
                        <div>
                            ...
                            <input type="date" name="Issue_Date" id="Issue_Date" value="{{ old('Issue_Date', !empty($visa['Issue_Date']) ? \Carbon\Carbon::parse($visa['Issue_Date'])->format('Y-m-d') : '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">
                        ...
                        <div>
                            ...
                            <input type="date" name="Expiry_Date" id="Expiry_Date" value="{{ old('Expiry_Date', !empty($visa['Expiry_Date']) ? \Carbon\Carbon::parse($visa['Expiry_Date'])->format('Y-m-d') : '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select name="Visa_Status" id="Visa_Status" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" required>
                                <option value="PREPARING" {{ old('Visa_Status', $visa['Visa_Status'] ?? '') == 'PREPARING' ? 'selected' : '' }}>PREPARING (Persiapan Berkas)</option>
                                <option value="SUBMITTED" {{ old('Visa_Status', $visa['Visa_Status'] ?? '') == 'SUBMITTED' ? 'selected' : '' }}>SUBMITTED (Diajukan)</option>
                                <option value="APPROVED" {{ old('Visa_Status', $visa['Visa_Status'] ?? '') == 'APPROVED' ? 'selected' : '' }}>APPROVED (Disetujui / Terbit)</option>
                                <option value="REJECTED" {{ old('Visa_Status', $visa['Visa_Status'] ?? '') == 'REJECTED' ? 'selected' : '' }}>REJECTED (Ditolak)</option>
                            </select>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <textarea name="Remarks" id="Remarks" rows="2" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">{{ old('Remarks', $visa['Remarks'] ?? '') }}</textarea>
                        </div>

                        <div>
                            <label for="Notes" class="block text-sm font-bold text-gray-700 mb-2">Catatan Internal WMS</label>
                            <textarea name="Notes" id="Notes" rows="2" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">{{ old('Notes', $visa['Notes'] ?? '') }}</textarea>
                        </div>
                    </div>
                </div>
