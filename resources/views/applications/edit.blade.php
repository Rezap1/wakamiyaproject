...
@section('header', 'Edit Data Aplikasi')
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 md:p-8 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-2xl font-extrabold text-gray-900">Edit Data Aplikasi</h2>
                    <p class="text-sm font-medium text-gray-500 mt-1">Ubah status aplikasi, info pembayaran, atau data untuk aplikasi <span class="font-bold text-gray-700">{{ $application['Application_Number'] ?? $application['Application_ID'] }}</span>.</p>
                </div>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('applications.show', $application['Application_ID']) }}" class="inline-flex items-center justify-center px-4 py-2 border border-gray-200 rounded-xl shadow-sm text-sm font-bold text-gray-700 bg-white hover:bg-gray-50 transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
        ...
        <div class="p-6 md:p-8">
            <form action="{{ route('applications.update', $application['Application_ID']) }}" method="POST" class="space-y-10">
                @csrf
                @method('PUT')
                        ...
                        <div>
                            ...
                            <select name="Student_ID" id="Student_ID" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" required>
                                ...
                                @foreach($students as $student)
                                    <option value="{{ $student['Student_ID'] }}" {{ old('Student_ID', $application['Student_ID'] ?? '') == $student['Student_ID'] ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="Job_Order_ID" id="Job_Order_ID" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" required>
                                ...
                                @foreach($jobOrders as $jo)
                                    <option value="{{ $jo['Job_Order_ID'] }}" {{ old('Job_Order_ID', $application['Job_Order_ID'] ?? '') == $jo['Job_Order_ID'] ? 'selected' : '' }}>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select name="Matching_ID" id="Matching_ID" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">
                                ...
                                @foreach($matchings as $matching)
                                    <option value="{{ $matching['Matching_ID'] }}" {{ old('Matching_ID', $application['Matching_ID'] ?? '') == $matching['Matching_ID'] ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div class="lg:col-span-2">
                            ...
                            <input type="text" name="Application_Number" id="Application_Number" value="{{ old('Application_Number', $application['Application_Number'] ?? '') }}" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" placeholder="Cth: APP-2023-001">
                        ...
                        <div>
                            ...
                            <select name="Application_Status" id="Application_Status" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" required>
                                <option value="SUBMITTED" {{ old('Application_Status', $application['Application_Status'] ?? '') == 'SUBMITTED' ? 'selected' : '' }}>Submitted (Diajukan)</option>
                                <option value="IN_PROGRESS" {{ old('Application_Status', $application['Application_Status'] ?? '') == 'IN_PROGRESS' ? 'selected' : '' }}>In Progress (Diproses)</option>
                                <option value="APPROVED" {{ old('Application_Status', $application['Application_Status'] ?? '') == 'APPROVED' ? 'selected' : '' }}>Approved (Diterima)</option>
                                <option value="REJECTED" {{ old('Application_Status', $application['Application_Status'] ?? '') == 'REJECTED' ? 'selected' : '' }}>Rejected (Ditolak)</option>
                                <option value="CANCELED" {{ old('Application_Status', $application['Application_Status'] ?? '') == 'CANCELED' ? 'selected' : '' }}>Canceled (Dibatalkan)</option>
                            </select>
                        ...
                        <div class="lg:col-span-2">
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="Application_Fee" id="Application_Fee" value="{{ old('Application_Fee', $application['Application_Fee'] ?? '') }}" min="0" step="1000" class="block w-full pl-10 rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" placeholder="0">
                            </div>
                        ...
                        <div class="lg:col-span-2">
                            ...
                            <select name="Payment_Status" id="Payment_Status" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">
                                ...
                                <option value="PAID" {{ old('Payment_Status', $application['Payment_Status'] ?? '') == 'PAID' ? 'selected' : '' }}>Paid (Lunas)</option>
                                <option value="PARTIAL" {{ old('Payment_Status', $application['Payment_Status'] ?? '') == 'PARTIAL' ? 'selected' : '' }}>Partial (Sebagian)</option>
                                <option value="FAILED" {{ old('Payment_Status', $application['Payment_Status'] ?? '') == 'FAILED' ? 'selected' : '' }}>Failed</option>
                                <option value="REFUNDED" {{ old('Payment_Status', $application['Payment_Status'] ?? '') == 'REFUNDED' ? 'selected' : '' }}>Refunded</option>
                            </select>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <textarea name="Remarks" id="Remarks" rows="3" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" placeholder="Keterangan aplikasi, syarat visa, dll.">{{ old('Remarks', $application['Remarks'] ?? '') }}</textarea>
                        </div>

                        <div>
                            <label for="Notes" class="block text-sm font-bold text-gray-700 mb-2">Catatan Internal WMS</label>
                            <textarea name="Notes" id="Notes" rows="2" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">{{ old('Notes', $application['Notes'] ?? '') }}</textarea>
                        </div>
                    </div>
                </div>
