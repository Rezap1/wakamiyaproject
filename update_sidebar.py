...
file_path = r'd:\orderan\wakamiya\resources\views\components\dashboard\sidebar.blade.php'
...
# Replace logo area
old_logo = '''    <div class="pt-8 pb-6 flex flex-col items-center justify-center border-b border-slate-800/50 bg-[#0F172A] relative">
...
new_logo = '''    <div class="pt-10 pb-8 flex flex-col items-center justify-center border-b border-slate-800/50 relative">
...
# Replace active menu classes (regex-like replacement for consistency)

# Match lines like: class="flex items-center px-4 py-3 text-sm font-medium rounded-xl {{ request()->routeIs('...') ? 'bg-gradient-to-r from-blue-800 to-blue-600 text-white shadow-lg shadow-blue-900/50' : 'text-slate-400 hover:bg-white/5 hover:text-white' }} transition-all"
pattern1 = r'class="flex items-center px-4 py-3 text-sm font-medium rounded-xl \{\{ request\(\)->routeIs\((.*?)\) \? \'bg-gradient-to-r from-blue-800 to-blue-600 text-white shadow-lg shadow-blue-900/50\' : \'text-slate-400 hover:bg-white/5 hover:text-white\' \}\} transition-all( relative)?"'
replacement1 = r'class="flex items-center pl-8 pr-4 py-3.5 text-[14px] font-medium mr-4 rounded-r-2xl \{\{ request()->routeIs(\1) ? \'bg-gradient-to-r from-[#1d4ed8] to-[#3b82f6] text-white shadow-md shadow-blue-900/40\' : \'text-slate-400 hover:bg-white/5 hover:text-white\' \}\} transition-all\2"'
content = re.sub(pattern1, replacement1, content)

pattern2 = r'class="flex items-center px-4 py-3 text-sm font-medium rounded-xl text-slate-400 hover:bg-slate-800 hover:text-white transition-all( relative)?"'
replacement2 = r'class="flex items-center pl-8 pr-4 py-3.5 text-[14px] font-medium mr-4 rounded-r-2xl text-slate-400 hover:bg-white/5 hover:text-white transition-all\1"'
content = re.sub(pattern2, replacement2, content)

# Profile area at bottom
old_profile = '''    <div class="p-4 bg-[#0F172A] border-t border-slate-800/50 mt-auto">
...
new_profile = '''    <div class="p-5 bg-[#0B1120] border-t border-slate-800/50 mt-auto">
...
pattern3 = r'<div class="px-2 mb-2 mt-4 text-xs font-bold text-slate-500 uppercase tracking-wider">(.*?)</div>'
replacement3 = r'<div class="pl-8 pr-4 mb-2 mt-6 text-[11px] font-bold text-slate-500 uppercase tracking-widest">\1</div>'
content = re.sub(pattern3, replacement3, content)

pattern4 = r'<div class="px-2 mt-4 mb-2 text-xs font-bold text-slate-500 uppercase tracking-wider">(.*?)</div>'
replacement4 = r'<div class="pl-8 pr-4 mt-6 mb-2 text-[11px] font-bold text-slate-500 uppercase tracking-widest">\1</div>'
content = re.sub(pattern4, replacement4, content)

pattern5 = r'<div class="px-2 mb-2 mt-8 text-xs font-bold text-slate-500 uppercase tracking-wider border-t border-slate-800 pt-4">(.*?)</div>'
replacement5 = r'<div class="pl-8 pr-4 mb-2 mt-8 text-[11px] font-bold text-slate-500 uppercase tracking-widest border-t border-slate-800/50 pt-6">\1</div>'
