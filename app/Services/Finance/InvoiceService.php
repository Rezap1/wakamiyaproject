...
class InvoiceService
{
    ...
    private function freshPaymentSnapshot(): Collection
    {
    ...
    /**
     * Preserve official education-invoice evidence across lifecycle changes.
     * Conflicting duplicate invoice identities fail closed.
     *
     * @return array<string, array>
     */
    public function educationInvoicesById(iterable $invoices): array
    {
        $unique = [];
        foreach ($invoices as $invoice) {
            ...
            if (isset($unique[$invoiceId]) && $unique[$invoiceId] !== $invoice) {
                throw new FinancialIntegrityException("Invoice_ID #{$invoiceId} duplikat dengan data berbeda.");
            }
            $unique[$invoiceId] = $invoice;
        }

        return array_filter($unique, fn ($invoice) => strtoupper(trim((string) ($invoice['Invoice_Type'] ?? 'STUDENT'))) === 'STUDENT'
            && $this->isEducationInvoice($invoice));
    }

    public static function educationCapacityLockKey(string $studentId): string
    {
        return 'finance_education_capacity_student_'.hash('sha256', trim($studentId));
    }

    /**
     * Canonical education-payment capacity. Verified money is paid; pending
     * money is only reserved so concurrent submissions cannot over-commit.
     */
    public function getStudentEducationPaymentState(
        string $studentId,
        ...
        $educationInvoicesById = $this->educationInvoicesById($invoiceRows);
        $snapshot = AcceptedPaymentCalculator::educationSnapshot(
            $paymentRows,
            $educationInvoicesById,
            $invoiceRows->pluck('Invoice_ID')->filter(fn ($id) => trim((string) $id) !== '')->all(),
        );
        $verifiedPaid = (float) ($snapshot['totals_by_student'][$studentId] ?? 0.0);
        $excludePaymentId = strtoupper(trim((string) $excludePaymentId));
        $studentPayments = collect($snapshot['payments'])
            ->filter(fn ($payment) => trim((string) ($payment['Student_ID'] ?? '')) === $studentId)
            ->filter(fn ($payment) => strtoupper(trim((string) ($payment['Payment_ID'] ?? ''))) !== $excludePaymentId);
        $pendingPayments = $studentPayments->filter(fn ($payment) => in_array(
            PaymentStatus::canonical($payment['Status'] ?? null),
            ['Waiting Verification', 'Need Revision'],
            true,
        ));
        $pendingReservedCents = $pendingPayments->sum(fn ($payment) => Money::cents(
            $payment['Amount_Paid'] ?? null,
            'Nominal pembayaran pendidikan pending',
            false,
        ));
        $pendingSelfServiceCents = $pendingPayments
            ->filter(fn ($payment) => trim((string) ($payment['Invoice_ID'] ?? '')) === '')
            ->sum(fn ($payment) => Money::cents(
                $payment['Amount_Paid'] ?? null,
                'Nominal Bayar Mandiri pending',
                false,
            ));
        ...
        return [
            ...
            'verified_payment_count' => $studentPayments
        ...
        if ($student) {
            if (! empty($student['Program_ID'])) {
                $program = $programSnapshot !== null
                    ...
                    : app(ProgramRepositoryInterface::class)->findById($student['Program_ID']);
            ...
            if (! empty($student['Batch_ID'])) {
                $batch = $batchSnapshot !== null
                    ...
                    : app(BatchRepositoryInterface::class)->findById($student['Batch_ID']);
    ...
    public function getStudentEducationBillingSummary(
        ...
        $paymentState = $this->getStudentEducationPaymentState(
            $studentId,
            $invoiceRows,
            $paymentRows,
            $studentSnapshot,
        ...
        return [
            ...
            'verified_education_paid' => $paymentState['verified_paid'],
    ...
    private function enforceEducationTuitionCap(array $data, ?string $excludeInvoiceId = null): void
    {
        ...
        $verifiedPaid = (float) $summary['verified_education_paid'];
        $pendingSelfService = (float) $summary['pending_self_service_reserved'];
        $outstandingInvoices = (float) $summary['remaining_to_pay'];
        ...
        if ($excludeInvoiceId !== null) {
            $paidOnCurrentInvoice = $this->getVerifiedPaymentTotal($excludeInvoiceId);
            $newOutstanding = max(0.0, round($newAmount - $paidOnCurrentInvoice, Money::SCALE));
        }
        $remaining = max(0.0, round(
            $tuitionFee - $verifiedPaid - $pendingSelfService - $outstandingInvoices,
            Money::SCALE,
        ));

        if (Money::cents($newOutstanding) > Money::cents($remaining)) {
            if (Money::cents($verifiedPaid) >= Money::cents($tuitionFee)) {
                throw new Exception('Biaya Pendidikan siswa sudah lunas. Invoice Biaya Pendidikan baru tidak dapat dibuat.');
            }
            throw new Exception(
                'Total invoice Biaya Pendidikan siswa ini tidak boleh melebihi plafon Rp '
                .number_format($tuitionFee, 0, ',', '.')
                .'. Sudah dibayar Rp '.number_format($verifiedPaid, 0, ',', '.')
    ...
    public function getInvoiceDocumentData(string $invoiceId, bool $allowPublicVerification = false): array
    {
        ...
        // QR Verification URL
        $verificationUrl = PublicVerificationUrl::make('invoices.verify-public', $invoiceId);
    ...
    public function create(array $data)
    {
        ...
        if ($this->isEducationInvoice($data)) {
        ...
        $run = function () use (&$data, $idempotencyKey, $fingerprint, $actorId, $requestId, $providedInvoiceId) {
            ...
            $executeWrite = function () use ($write, &$data) {
                if ($this->isEducationInvoice($data) && ! empty($data['Student_ID'])) {
                    $lockKey = self::educationCapacityLockKey((string) $data['Student_ID']);
    ...
    private function assertFinanceMutationActor(): string
    {
        ...
        if ($role === '' && $user && ! empty($user->Role_ID)) {
            try {
                $role = strtoupper(trim((string) (app(RoleService::class)
        ...
        if (! in_array($role, ['FINANCE', 'ADMINISTRATOR', 'MASTER'], true)) {
            throw new AuthorizationException('Role pengguna tidak diizinkan melakukan mutasi invoice.');
        }
    ...
    public function update($id, array $data)
    {
        ...
        $data['Updated_By'] = ActorIdentity::required();
