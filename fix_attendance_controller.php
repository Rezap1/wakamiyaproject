...
public function index(Request $request)
    {
        ...
        // Filter attendances
        $attendances = $attendances->filter(function($a) use ($dateFilter, $dateEndFilter, $classFilter, $statusFilter, $search, $students, $schedules) {
            try {
                ...
                // Resolve Class_ID from Schedule_ID if necessary
                $cId = $a['Class_ID'] ?? '';
                ...
                $a['Resolved_Class_ID'] = $cId; // Store it for grouping later
                ...
                // Save the resolved class back to the item so groupBy can use it
                return $a;
            } catch (\Exception $e) {
                return false;
            }
